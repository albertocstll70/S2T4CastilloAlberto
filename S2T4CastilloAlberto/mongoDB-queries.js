...
//2.- Escriu una consulta per mostrar el restaurant_id, name, borough i cuisine per tots els documents en la col·lecció
//db.restaurants.find().forEach(restaurant => print("name: "+restaurant.name+ ",  borough:  "+restaurant.gorough +",   cuisine: "+restaurant.cuisine) )
...
db.restaurants.find({},{restaurant_id:1,name:1,borough:1,_id:0,address:{zipcode:1}}).pretty();
...
db.restaurants.find({borough:"Bronx"},{name:1,_id:0}).limit(5).skip(5).pretty();

//8.- Escriu una consulta per trobar els restaurants que tenen un score de més de 90.
db.restaurants.find({"grades.score":{$gt:90}},{"name":1, _id:0,"grades.score":1}).pretty();

//9.- Escriu una consulta per trobar els restaurants que tenen un score de més de 80 però menys que 100.
db.restaurants.find({"grades.score":{$gt:80,$lt:100}},{"name":1, _id:0,"grades.score":1}).pretty();
...
db.restaurants.find({$and:[{"cuisine":{$ne:"American"},"grades.score":{$gt:70},"address.coord":{$lt:-65.754168}}]},{"name":1,cuisine:1,_id:0}).pretty();
...
db.restaurants.find({"cuisine":{$ne:"American"},"grades.score":{$lt:70},"address.coord":{$gt:-65.754168}},{"name":1,cuisine:1,_id:0}).pretty();
...
db.restaurants.find({$and:[{"cuisine":{$ne:"American"},"grades.grade":{$eq:"A"},"borough":{$ne:"Brooklyn"}}]},{_id:0,"name":1,"cuisine":1}).sort({cuisine:-1}).pretty();
...
db.restaurants.find({"name":/^Wil/},{"restaurant_id":1,"name":1,"cuisine":1,_id:0}).pretty();
...
db.restaurants.find({$and:[{borough:"Bronx"},{$or:[{cuisine:"American"},{cuisine:"Chinese"}]}]},{name:1,cuisine:1,_id:0}).pretty();
...
db.restaurants.find({$nor:[{borough:"Staten Island"},{borough:"Queens"},{borough:"Bronx"},{borough:"Brooklyn"}]},{restaurant_id:1,name:1,borough:1,cuisine:1,_id:0}).pretty();
...
db.restaurants.find({$and:[{name:/Fish/},{borough:{$ne:"American"}},{borough:{$ne:"Chinese"}},{ame:{$not:/^Wil/}}]},{restaurat_id:1,name:1,borough:1,cuisine:1,_id:0}).pretty();
...
db.restaurants.find({$and:[{"grades.grade":"A"},{"grades.score":11},{"grades.date":ISODate("2014-08-11T00:00:00Z")}]},{restaurant_id:1,name:1,grades:1,_id:0}).pretty();
...
db.restaurants.find({$and:[{"grades.1.grade":"A"},{"grades.1.score":9},{"grades.1.date":ISODate("2014-08-11T00:00:00Z")}]},{restaurant_id:1,name:1,_id:0}).pretty();
...
db.restaurants.find().pretty().sort({cuisine:1, borough:-1});
